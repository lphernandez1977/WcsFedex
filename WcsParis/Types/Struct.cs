...
namespace WcsFedex.Types
{
    public static class Struct
    {
                ...
                switch (info.FieldType.Name)
                {
                    ...
                    case "UInt16":
                        ...
                        if ((numBytes / 2 - Math.Floor(numBytes / 2.0)) > 0)
                            numBytes++;
                        // hier auswerten
                        info.SetValue(structValue, WcsFedex.Types.Word.FromBytes(bytes[(int)numBytes + 1],
                                                                          bytes[(int)numBytes]));
                        numBytes += 2;
                        break;
                    case "Int32":
                    case "UInt32":
                        ...
                        if ((numBytes / 2 - Math.Floor(numBytes / 2.0)) > 0)
                            numBytes++;
                        // hier auswerten
                        info.SetValue(structValue, WcsFedex.Types.DWord.FromBytes(bytes[(int)numBytes],
                    ...
                    case "Double":
                        ...
                        if ((numBytes / 2 - Math.Floor(numBytes / 2.0)) > 0)
                            numBytes++;
                        // hier auswerten
                        info.SetValue(structValue, WcsFedex.Types.Double.FromByteArray(new byte[] { bytes[(int)numBytes],
                                                                           bytes[(int)numBytes + 1],
                                                                           bytes[(int)numBytes + 2],
                                                                           bytes[(int)numBytes + 3] }));
                        numBytes += 4;
        ...
        public static byte[] ToBytes(object structValue)
        {
            ...
            int size = WcsFedex.Types.Struct.GetStructSize(type);
            ...
            foreach (System.Reflection.FieldInfo info in infos)
            {
                ...
                switch (info.FieldType.Name)
                {
                    ...
                    case "Byte":
                        ...
                        break;
                    case "Int16":
                        bytes2 = WcsFedex.Types.Int.ToByteArray((Int16)info.GetValue(structValue));
                        break;
                    case "UInt16":
                        bytes2 = WcsFedex.Types.Word.ToByteArray((UInt16)info.GetValue(structValue));
                        break;
                    case "Int32":
                        bytes2 = WcsFedex.Types.DInt.ToByteArray((Int32)info.GetValue(structValue));
                        break;
                    case "UInt32":
                        bytes2 = WcsFedex.Types.DWord.ToByteArray((UInt32)info.GetValue(structValue));
                        break;
                    case "Double":
                        bytes2 = WcsFedex.Types.Double.ToByteArray((double)info.GetValue(structValue));
                        break;
                }
                if (bytes2 != null)
                {
